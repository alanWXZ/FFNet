...
class NYUv2(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        item_idx_dep = str(int(item_idx)-1).zfill(4)
        depth=os.path.join('/media/ExtHDD02/wangxuzhi/data/NYU_CAD/NYU_depth_predicted_by_revisiting_NYUCAD/', item_idx_dep + '.npy')
        hha_path = os.path.join(self._hha_path, item_idx+'.png')
        #hha_path=os.path.join('/mnt/data/SSC_Dataset/NYUCAD_HHA/', item_idx+'.png')
        ...
        mapping_path = os.path.join(
            '/media/ExtHDD02/wangxuzhi/data/NYU_CAD/NYU_mapping_revisiting_low_resolution/',
            item_idx + '.npz')

        item_idx_00=item_idx.zfill(6)
        gt_path_2d=os.path.join('/media/ExtHDD02/wangxuzhi/nyu_vis_train/', item_idx_00 + '.png')
        #gt_path_2d = os.path.join('/home/ubuntu/date/hdd1/wangxuzhi/nyu_vis_train/',item_idx_00+'.png')
        item_name = item_idx



        img, hha, tsdf, label_weight, depth_mapping_3d, gt, sketch_gt,gt_2d,depth = self._fetch_data(img_path, hha_path, label_weight_path, mapping_path, gt_path,gt_path_2d,depth)
        ...
        if self._split_name is 'train':
            ...
            gt_2d = torch.from_numpy(np.ascontiguousarray(gt_2d)).float()
        ...
        output_dict = dict(data=img, label=gt, label_weight=label_weight, depth_mapping_3d=depth_mapping_3d,
                           tsdf=tsdf, sketch_gt=sketch_gt, gt_2d=gt_2d,depth=depth,fn=str(item_name), n=len(self._file_names))
        if self.preprocess is not None and extra_dict is not None:
            output_dict.update(**extra_dict)

        return output_dict

    def _fetch_data(self, img_path, hha_path, label_weight_path, mapping_path, gt_path,gt_path_2d,depth, dtype=None):
        from config import config
        ...
        tsdf = np.load(label_weight_path,allow_pickle=True)['arr_0'].astype(np.float32).reshape(1, 60, 36, 60)
        ...
        gt_2d= cv2.imread(gt_path_2d, -1).astype(np.int64)

        return img, hha, tsdf, label_weight, depth_mapping_3d, gt, sketch_gt,gt_2d,depth
